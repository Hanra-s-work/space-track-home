...
CONTAINER_NAME=space-track-home
CONTAINER_IMAGE=my-apache2-space-track-home
CONTAINER_PORT=80

SRC_FOLD=.
DEFAULT_DEST_DIR=/usr/local/apache2/htdocs
DOCKER_NEW_FOLD=space-track-home
DEST_FOLD=$DEFAULT_DEST_DIR/$DOCKER_NEW_FOLD
...
function create_redirect_file() {
    echo "Create redirect file ..."
    RES='<html>\n<head>\n'
    RES+="<title>Redirecting to '$DOCKER_NEW_FOLD'</title>\\n"
    RES+="<meta http-equiv='refresh' content=\\\"5; url='$DOCKER_NEW_FOLD'\\\">\\n"
    RES+='</head>\n<body>\n'
    RES+="<p>Redirecting to <a href=\\\"$DOCKER_NEW_FOLD\\\">$DOCKER_NEW_FOLD</a></p>\\n"
    RES+='</body>\n</html>\n'
    echo "RUN echo \"$RES\" >\"$DEFAULT_DEST_DIR/index.html\" && echo Displaying content && cat $DEFAULT_DEST_DIR/index.html" >>Dockerfile
